...
import { NOVA_LOCACAO_STAGES } from "@/lib/ticketPhases"
    ...
    <div className="bg-white border rounded-lg shadow-sm p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-start gap-4 md:gap-0">
        {STAGE_GROUPS.map((grupo, idx) => {
          ...
          const concluido = finalizado || faseAtual > maxFase
          const ativo = !finalizado && faseAtual >= minFase && faseAtual <= maxFase
            ...
            <div key={grupo.stage} className="flex-1 flex md:flex-col items-center">
              <div className="flex items-center w-full">
                ...
                <div className={`flex items-center justify-center w-9 h-9 rounded-full font-bold text-sm shrink-0 mx-1 border-2
                  ${concluido ? 'bg-green-500 border-green-500 text-white' : ativo ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-gray-300 text-gray-400'}`}>
                  {concluido ? <CheckCircle2 size={18} /> : grupo.stage}
                </div>
              ...
              <div className="ml-3 md:ml-0 md:mt-2 text-left md:text-center">
                <p className={`text-xs font-bold ${ativo ? 'text-amber-700' : concluido ? 'text-green-700' : 'text-gray-400'}`}>
                  {grupo.stageLabel}
                </p>
